...
def _grommets():
    ...
    return (
        nhp_apc.admission_has(
            primary_procedure, "D151"
        )  # ty: ignore[call-non-callable]
        .admission_has(any_procedure, "D151")  # ty: ignore[call-non-callable]
        .admission_has(
            primary_diagnosis, "H6(5[^01]|6)"
        )  # ty: ignore[call-non-callable]
        .filter(
            (F.col("age") <= 18) | ((F.col("age") >= 7001) & (F.col("age") <= 7007))
        )
